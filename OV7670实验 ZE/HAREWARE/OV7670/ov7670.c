...
u8 OV7670_Init(void)
{
	...
	GPIO_InitTypeDef  GPIO_InitStructure;
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOG|RCC_APB2Periph_AFIO, ENABLE);	 //使能端口时钟和复用时钟
  ...
  GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE);	//取消原来的复用 使其变为普通IO SWD
	
	SCCB_Init();                                   //初始化SCCB通信IO口
	
	if(SCCB_WR_Reg(0x12,0x80)) return 1;             //复位SCCB
	
	delay_ms(50);
	//读取ID
	temp=SCCB_RD_Reg(0x0b);              //产品低位ID识别号
	if(temp!=0x73)  return 2;           //核对ID
	temp=SCCB_RD_Reg(0x0a);             //高位
	if(temp!=0x76)  return 2;            //核对ID
	//初始化序列	  
