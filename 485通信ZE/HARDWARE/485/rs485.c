...
void USART2_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)    //接收到数据
	{
		...
		if(RS485_RX_CNT<64)
		{
			RS485_RX_BUF[RS485_RX_CNT]=res;                    //记录接收的数据
			RS485_RX_CNT++;
		}
	}
}
...
void RS485_Init(u32 bound)
{
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_7;           //SP3485的RE端
...
void RS485_Receive_Data(u8 *buf,u8 *len)
{
	...
	u8 i=0;
	*len=0;                      //默认为0
	delay_ms(10);                //超过10ms还未接收的数据 默认接收完毕
	if(rxlen==RS485_RX_CNT&&rxlen)          //如果接收到了数据 且接收完成
	{
		for(i=0;i<rxlen;i++)
		{
