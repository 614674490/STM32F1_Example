...
int main()
{
	...
	CAN_Mode_Init(CAN_SJW_1tq,CAN_BS2_8tq,CAN_BS1_9tq,4,CAN_Mode_Normal);  //初始化环回模式 波特率 500Kbps/s
	...
	while(1)
	{
		...
		else if(key==WKUP_PRES)    //改变CAN的工作模式
		{
			...
			printf("\r\nMODE:");
			if(mode==0) printf("Nnormal Mode\r\n");           //普通模式
			else if(mode==1) printf("LoopBack Mode\r\n");     //环回模式
		}
